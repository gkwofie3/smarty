...
import { t as require_scheduler } from "./scheduler-DjSBj4NM.js";
...
function getBezierExtremaPoints(points) {
	...
	for (const axis of axisPoints) {
		const a$1 = -3 * axis[0] + 9 * axis[1] - 9 * axis[2] + 3 * axis[3];
		if (a$1 !== 0) {
			const b$1 = 6 * axis[0] - 12 * axis[1] + 6 * axis[2];
			const c$1 = -3 * axis[0] + 3 * axis[1];
			const discriminant = b$1 * b$1 - 4 * a$1 * c$1;
			if (discriminant >= 0) {
				const d = Math.sqrt(discriminant);
				extremaTs.push((-b$1 + d) / (2 * a$1));
				extremaTs.push((-b$1 - d) / (2 * a$1));
			}
		}
	}
...
var Transformer$1 = class extends Group$1 {
	...
	_handleMouseMove(e) {
		...
		if (this._movingAnchorName === "rotater") {
			...
			let delta = Math.atan2(-y, x$1) + Math.PI / 2 - rotateAnchorAngleRad;
